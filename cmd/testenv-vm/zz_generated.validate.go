...
func ValidateDHCPSpec(s *v1.DHCPSpec) *mcptypes.ConfigValidateOutput {
	...
	var errors []mcptypes.ValidationError
	// Validate required field: rangeEnd
	if s.RangeEnd == "" {
		errors = append(errors, mcptypes.ValidationError{
			Field:   "spec.rangeEnd",
			Message: "required field is missing",
	...
	// Validate required field: rangeStart
	if s.RangeStart == "" {
		errors = append(errors, mcptypes.ValidationError{
			Field:   "spec.rangeStart",
			Message: "required field is missing",
		})
	}
...
func ValidateTFTPSpec(s *v1.TFTPSpec) *mcptypes.ConfigValidateOutput {
	...
	var errors []mcptypes.ValidationError
	// Validate required field: bootFile
	if s.BootFile == "" {
		errors = append(errors, mcptypes.ValidationError{
			Field:   "spec.bootFile",
			Message: "required field is missing",
...
func ValidateNetworkSpec(s *v1.NetworkSpec) *mcptypes.ConfigValidateOutput {
	...
	{
		nested := s.Dhcp
		nestedResult := ValidateDHCPSpec(&nested)
		if !nestedResult.Valid {
			for _, e := range nestedResult.Errors {
				errors = append(errors, mcptypes.ValidationError{
	...
	{
		nested := s.Dns
		nestedResult := ValidateDNSSpec(&nested)
		if !nestedResult.Valid {
			for _, e := range nestedResult.Errors {
				errors = append(errors, mcptypes.ValidationError{
	...
	{
		nested := s.Tftp
		nestedResult := ValidateTFTPSpec(&nested)
		if !nestedResult.Valid {
			for _, e := range nestedResult.Errors {
				errors = append(errors, mcptypes.ValidationError{

...
// CloudInitReadinessSpec represents the CloudInitReadinessSpec configuration.
// Cloud-init completion readiness check.
type CloudInitReadinessSpec struct {
	// Enables cloud-init completion check.
	Enabled bool `json:"enabled"`
	// Timeout for cloud-init to complete (e.g., 10m).
...
type DHCPSpec struct {
	...
	// Last IP in DHCP range. Required when enabled is true.
	RangeEnd string `json:"rangeEnd,omitempty"`
	// First IP in DHCP range. Required when enabled is true.
	RangeStart string `json:"rangeStart,omitempty"`
	// Custom gateway/router IP to advertise via DHCP. Defaults to network gateway.
...
type DiskSpec struct {
	...
	// Disk bus: virtio, sata, scsi or ide. Empty means virtio.
	Bus string `json:"bus,omitempty"`
	// Disk size (e.g., 20G).
	Size string `json:"size"`
	// World wide name of the disk, 16 hex digits. Needs bus sata, scsi or ide.
	Wwn string `json:"wwn,omitempty"`
}

// ImageCustomizeSpec represents the ImageCustomizeSpec configuration.
// Offline image customization via virt-customize. When set, CacheManager builds a pre-customized qcow2 overlay.
type ImageCustomizeSpec struct {
	// APT packages to pre-install via virt-customize --install.
	Packages []string `json:"packages,omitempty"`
	// Shell commands to execute offline via virt-customize --run-command.
...
type TFTPSpec struct {
	// Default boot file (e.g., undionly.kpxe). Required when enabled is true.
	BootFile string `json:"bootFile,omitempty"`
	// Enables TFTP server.
	Enabled bool `json:"enabled,omitempty"`
	// Directory for TFTP files. Required when enabled is true.
...
// TCPReadinessSpec represents the TCPReadinessSpec configuration.
// TCP port readiness check configuration.
type TCPReadinessSpec struct {
	// Address to dial when the guest holds a static address the host cannot learn from a DHCP lease. The provider reports it as the VM address.
	Address string `json:"address,omitempty"`
	// Port to check for TCP connectivity.
	Port int `json:"port"`
	// Timeout for port to become available (e.g., 5m).
	Timeout string `json:"timeout,omitempty"`
}
...
type ImageSpec struct {
	...
	Customize *ImageCustomizeSpec `json:"customize,omitempty"`
...
type VMSpec struct {
	...
	// Path to an ISO attached as a cdrom device. A template such as {{ .Images.name.Path }} resolves it through the image cache.
	Cdrom     string        `json:"cdrom,omitempty"`
	CloudInit CloudInitSpec `json:"cloudInit,omitempty"`
	Disk      DiskSpec      `json:"disk"`
	// Memory in MB.
	Memory int `json:"memory"`
	// Name of the network resource to attach. Deprecated in favor of networks.
	Network string `json:"network,omitempty"`
	// List of network resource names to attach. Takes precedence over network.
...
func BootSpecFromMap(m map[string]interface{}) (*BootSpec, error) {
	...
	for key := range m {
		...
		case "order":
		default:
			return nil, fmt.Errorf("field %s: not a key of BootSpec; the keys are firmware, order", key)
		}
	}
	// Parse firmware
...
func CloudInitNameserversFromMap(m map[string]interface{}) (*CloudInitNameservers, error) {
	...
	for key := range m {
		...
		case "addresses":
		default:
			return nil, fmt.Errorf("field %s: not a key of CloudInitNameservers; the keys are addresses", key)
		}
	}
	// Parse addresses
...
// CloudInitReadinessSpecFromMap creates a CloudInitReadinessSpec from a map[string]interface{}.
func CloudInitReadinessSpecFromMap(m map[string]interface{}) (*CloudInitReadinessSpec, error) {
	if m == nil {
		return &CloudInitReadinessSpec{}, nil
	}

	s := &CloudInitReadinessSpec{}
	...
	for key := range m {
		...
		case "timeout":
		default:
			return nil, fmt.Errorf("field %s: not a key of CloudInitReadinessSpec; the keys are enabled, timeout", key)
		}
	}
	// Parse enabled
...
func UserSpecFromMap(m map[string]interface{}) (*UserSpec, error) {
	...
	for key := range m {
		...
		case "sudo":
		default:
			return nil, fmt.Errorf("field %s: not a key of UserSpec; the keys are name, sshAuthorizedKeys, sudo", key)
		}
	}
	// Parse name
...
func WriteFileSpecFromMap(m map[string]interface{}) (*WriteFileSpec, error) {
	...
	for key := range m {
		...
		case "permissions":
		default:
			return nil, fmt.Errorf("field %s: not a key of WriteFileSpec; the keys are content, path, permissions", key)
		}
	}
	// Parse content
...
func DHCPSpecFromMap(m map[string]interface{}) (*DHCPSpec, error) {
	...
	for key := range m {
		...
		case "router":
		default:
			return nil, fmt.Errorf("field %s: not a key of DHCPSpec; the keys are dnsServers, enabled, leaseTime, rangeEnd, rangeStart, router", key)
		}
	}
	// Parse dnsServers
	if v, ok := m["dnsServers"]; ok && v != nil {
		if arr, ok := v.([]interface{}); ok {
			s.DnsServers = make([]string, 0, len(arr))
			for i, item := range arr {
				if str, ok := item.(string); ok {
					s.DnsServers = append(s.DnsServers, str)
				} else {
					return nil, fmt.Errorf("field dnsServers[%d]: expected string, got %T", i, item)
				}
			}
		} else if arr, ok := v.([]string); ok {
			s.DnsServers = arr
		} else {
			return nil, fmt.Errorf("field dnsServers: expected []string, got %T", v)
		}
	}
	// Parse enabled
	if v, ok := m["enabled"]; ok && v != nil {
	...
	// Parse router
	if v, ok := m["router"]; ok && v != nil {
		if val, ok := v.(string); ok {
			s.Router = val
		} else {
			return nil, fmt.Errorf("field router: expected string, got %T", v)
		}
	}
	return s, nil
}
...
func DNSSpecFromMap(m map[string]interface{}) (*DNSSpec, error) {
	...
	for key := range m {
		...
		case "servers":
		default:
			return nil, fmt.Errorf("field %s: not a key of DNSSpec; the keys are enabled, servers", key)
		}
	}
	// Parse enabled
	if v, ok := m["enabled"]; ok && v != nil {
...
func DiskSpecFromMap(m map[string]interface{}) (*DiskSpec, error) {
	...
	for key := range m {
		...
		case "wwn":
		default:
			return nil, fmt.Errorf("field %s: not a key of DiskSpec; the keys are baseImage, bus, size, wwn", key)
		}
	}
	// Parse baseImage
	...
	// Parse bus
	if v, ok := m["bus"]; ok && v != nil {
		if val, ok := v.(string); ok {
			s.Bus = val
		} else {
			return nil, fmt.Errorf("field bus: expected string, got %T", v)
		}
	}
	// Parse size
	...
	// Parse wwn
	if v, ok := m["wwn"]; ok && v != nil {
		if val, ok := v.(string); ok {
			s.Wwn = val
		} else {
			return nil, fmt.Errorf("field wwn: expected string, got %T", v)
		}
	}
	return s, nil
}

// ImageCustomizeSpecFromMap creates a ImageCustomizeSpec from a map[string]interface{}.
func ImageCustomizeSpecFromMap(m map[string]interface{}) (*ImageCustomizeSpec, error) {
	if m == nil {
		return &ImageCustomizeSpec{}, nil
	}

	s := &ImageCustomizeSpec{}
	...
	for key := range m {
		...
		case "runcmd":
		default:
			return nil, fmt.Errorf("field %s: not a key of ImageCustomizeSpec; the keys are packages, runcmd", key)
		}
	}
	// Parse packages
...
func KeySpecFromMap(m map[string]interface{}) (*KeySpec, error) {
	...
	for key := range m {
		...
		case "type":
		default:
			return nil, fmt.Errorf("field %s: not a key of KeySpec; the keys are bits, comment, outputDir, type", key)
		}
	}
	// Parse bits
...
func TFTPSpecFromMap(m map[string]interface{}) (*TFTPSpec, error) {
	...
	for key := range m {
		...
		case "root":
		default:
			return nil, fmt.Errorf("field %s: not a key of TFTPSpec; the keys are bootFile, enabled, root", key)
		}
	}
	// Parse bootFile
...
func ProviderConfigFromMap(m map[string]interface{}) (*ProviderConfig, error) {
	...
	for key := range m {
		...
		case "spec":
		default:
			return nil, fmt.Errorf("field %s: not a key of ProviderConfig; the keys are default, engine, name, spec", key)
		}
	}
	// Parse default
...
func SSHReadinessSpecFromMap(m map[string]interface{}) (*SSHReadinessSpec, error) {
	...
	for key := range m {
		...
		case "user":
		default:
			return nil, fmt.Errorf("field %s: not a key of SSHReadinessSpec; the keys are enabled, privateKey, timeout, user", key)
		}
	}
	// Parse enabled
	if v, ok := m["enabled"]; ok && v != nil {
...
// TCPReadinessSpecFromMap creates a TCPReadinessSpec from a map[string]interface{}.
func TCPReadinessSpecFromMap(m map[string]interface{}) (*TCPReadinessSpec, error) {
	if m == nil {
		return &TCPReadinessSpec{}, nil
	}
	...
	for key := range m {
		...
		case "timeout":
		default:
			return nil, fmt.Errorf("field %s: not a key of TCPReadinessSpec; the keys are address, port, timeout", key)
		}
	}
	// Parse address
	if v, ok := m["address"]; ok && v != nil {
		if val, ok := v.(string); ok {
			s.Address = val
		} else {
			return nil, fmt.Errorf("field address: expected string, got %T", v)
		}
	}
	// Parse port
	if v, ok := m["port"]; ok && v != nil {
		...
		case float64:
			s.Port = int(val)
		default:
			return nil, fmt.Errorf("field port: expected int, got %T", v)
		}
	}
	// Parse timeout
	if v, ok := m["timeout"]; ok && v != nil {
...
func ResourceRefFromMap(m map[string]interface{}) (*ResourceRef, error) {
	...
	for key := range m {
		...
		case "provider":
		default:
			return nil, fmt.Errorf("field %s: not a key of ResourceRef; the keys are kind, name, provider", key)
		}
	}
	// Parse kind
...
func CloudInitEthernetConfigFromMap(m map[string]interface{}) (*CloudInitEthernetConfig, error) {
	...
	for key := range m {
		...
		case "nameservers":
		default:
			return nil, fmt.Errorf("field %s: not a key of CloudInitEthernetConfig; the keys are addresses, dhcp4, gateway4, name, nameservers", key)
		}
	}
	// Parse addresses
	if v, ok := m["addresses"]; ok && v != nil {
...
func ImageSpecFromMap(m map[string]interface{}) (*ImageSpec, error) {
	...
	for key := range m {
		...
		case "source":
		default:
			return nil, fmt.Errorf("field %s: not a key of ImageSpec; the keys are alias, customize, sha256, source", key)
		}
	}
	// Parse alias
	...
	// Parse customize
	if v, ok := m["customize"]; ok && v != nil {
		if obj, ok := v.(map[string]interface{}); ok {
			ref, err := ImageCustomizeSpecFromMap(obj)
			if err != nil {
				return nil, fmt.Errorf("field customize: %w", err)
			}
			s.Customize = ref
		} else {
			return nil, fmt.Errorf("field customize: expected object, got %T", v)
		}
	}
	// Parse sha256
...
func KeyResourceFromMap(m map[string]interface{}) (*KeyResource, error) {
	...
	for key := range m {
		...
		case "spec":
		default:
			return nil, fmt.Errorf("field %s: not a key of KeyResource; the keys are name, provider, providerSpec, spec", key)
		}
	}
	// Parse name
	if v, ok := m["name"]; ok && v != nil {
...
func NetworkSpecFromMap(m map[string]interface{}) (*NetworkSpec, error) {
	...
	for key := range m {
		...
		case "tftp":
		default:
			return nil, fmt.Errorf("field %s: not a key of NetworkSpec; the keys are attachTo, cidr, dhcp, dns, gateway, mtu, tftp", key)
		}
	}
	// Parse attachTo
...
func ReadinessSpecFromMap(m map[string]interface{}) (*ReadinessSpec, error) {
	...
	for key := range m {
		...
		case "tcp":
		default:
			return nil, fmt.Errorf("field %s: not a key of ReadinessSpec; the keys are cloudInit, ssh, tcp", key)
		}
	}
	// Parse cloudInit
	if v, ok := m["cloudInit"]; ok && v != nil {
		if obj, ok := v.(map[string]interface{}); ok {
			ref, err := CloudInitReadinessSpecFromMap(obj)
			if err != nil {
	...
	// Parse tcp
	if v, ok := m["tcp"]; ok && v != nil {
		if obj, ok := v.(map[string]interface{}); ok {
			ref, err := TCPReadinessSpecFromMap(obj)
			if err != nil {
				return nil, fmt.Errorf("field tcp: %w", err)
			}
			if ref != nil {
				s.Tcp = *ref
			}
		} else {
			return nil, fmt.Errorf("field tcp: expected object, got %T", v)
		}
	}
	return s, nil
}
...
func CloudInitNetworkConfigFromMap(m map[string]interface{}) (*CloudInitNetworkConfig, error) {
	...
	for key := range m {
		...
		case "ethernets":
		default:
			return nil, fmt.Errorf("field %s: not a key of CloudInitNetworkConfig; the keys are ethernets", key)
		}
	}
	// Parse ethernets
...
func ImageResourceFromMap(m map[string]interface{}) (*ImageResource, error) {
	...
	for key := range m {
		...
		case "spec":
		default:
			return nil, fmt.Errorf("field %s: not a key of ImageResource; the keys are name, spec", key)
		}
	}
	// Parse name
	if v, ok := m["name"]; ok && v != nil {
...
func NetworkResourceFromMap(m map[string]interface{}) (*NetworkResource, error) {
	...
	for key := range m {
		...
		case "spec":
		default:
			return nil, fmt.Errorf("field %s: not a key of NetworkResource; the keys are kind, name, provider, providerSpec, spec", key)
		}
	}
	// Parse kind
	if v, ok := m["kind"]; ok && v != nil {
...
func CloudInitSpecFromMap(m map[string]interface{}) (*CloudInitSpec, error) {
	...
	for key := range m {
		...
		case "writeFiles":
		default:
			return nil, fmt.Errorf("field %s: not a key of CloudInitSpec; the keys are hostname, networkConfig, packages, runcmd, users, writeFiles", key)
		}
	}
	// Parse hostname
...
func VMSpecFromMap(m map[string]interface{}) (*VMSpec, error) {
	...
	for key := range m {
		...
		case "vcpus":
		default:
			return nil, fmt.Errorf("field %s: not a key of VMSpec; the keys are boot, cdrom, cloudInit, disk, memory, network, networks, readiness, vcpus", key)
		}
	}
	// Parse boot
	...
	// Parse cdrom
	if v, ok := m["cdrom"]; ok && v != nil {
		if val, ok := v.(string); ok {
			s.Cdrom = val
		} else {
			return nil, fmt.Errorf("field cdrom: expected string, got %T", v)
		}
	}
	// Parse cloudInit
	if v, ok := m["cloudInit"]; ok && v != nil {
	...
	if v, ok := m["networks"]; ok && v != nil {
		if arr, ok := v.([]interface{}); ok {
			s.Networks = make([]string, 0, len(arr))
			for i, item := range arr {
				if str, ok := item.(string); ok {
					s.Networks = append(s.Networks, str)
				} else {
					return nil, fmt.Errorf("field networks[%d]: expected string, got %T", i, item)
				}
			}
		} else if arr, ok := v.([]string); ok {
			s.Networks = arr
		} else {
			return nil, fmt.Errorf("field networks: expected []string, got %T", v)
		}
	}
	// Parse readiness
...
func VMResourceFromMap(m map[string]interface{}) (*VMResource, error) {
	...
	for key := range m {
		...
		case "spec":
		default:
			return nil, fmt.Errorf("field %s: not a key of VMResource; the keys are name, provider, providerSpec, spec", key)
		}
	}
	// Parse name
	if v, ok := m["name"]; ok && v != nil {
...
func SpecFromMap(m map[string]interface{}) (*Spec, error) {
	...
	for key := range m {
		...
		case "vms":
		default:
			return nil, fmt.Errorf("field %s: not a key of Spec; the keys are artifactDir, cleanupOnFailure, defaultBaseImage, defaultProvider, imageCacheDir, images, keys, networks, providers, stateDir, vms", key)
		}
	}
	// Parse artifactDir
...
// ToMap converts a CloudInitReadinessSpec to a map[string]interface{}.
func (s *CloudInitReadinessSpec) ToMap() map[string]interface{} {
	if s == nil {
		return nil
	}
...
// ToMap converts a ImageCustomizeSpec to a map[string]interface{}.
func (s *ImageCustomizeSpec) ToMap() map[string]interface{} {
	if s == nil {
		return nil
	}
...
// ToMap converts a TCPReadinessSpec to a map[string]interface{}.
func (s *TCPReadinessSpec) ToMap() map[string]interface{} {
	if s == nil {
		return nil
	}
...
func (s *CloudInitEthernetConfig) ToMap() map[string]interface{} {
	...
	if refMap := s.Nameservers.ToMap(); len(refMap) > 0 {
...
func (s *ReadinessSpec) ToMap() map[string]interface{} {
	...
	m := make(map[string]interface{})
	// Reference type CloudInitReadinessSpec
	if refMap := s.CloudInit.ToMap(); len(refMap) > 0 {
		m["cloudInit"] = refMap
	}
	// Reference type SSHReadinessSpec
	if refMap := s.Ssh.ToMap(); len(refMap) > 0 {
		m["ssh"] = refMap
	}
	// Reference type TCPReadinessSpec
	if refMap := s.Tcp.ToMap(); len(refMap) > 0 {
		m["tcp"] = refMap
	}
	return m
}
...
func (s *CloudInitSpec) ToMap() map[string]interface{} {
	...
	if refMap := s.NetworkConfig.ToMap(); len(refMap) > 0 {
...
func (s *VMSpec) ToMap() map[string]interface{} {
	...
	if refMap := s.Boot.ToMap(); len(refMap) > 0 {
		m["boot"] = refMap
	}
	if s.Cdrom != "" {
		m["cdrom"] = s.Cdrom
	...
	if refMap := s.Disk.ToMap(); len(refMap) > 0 {
		m["disk"] = refMap
	}
	if s.Memory != 0 {
		m["memory"] = s.Memory
	...
	if refMap := s.Readiness.ToMap(); len(refMap) > 0 {
		m["readiness"] = refMap
	}
	if s.Vcpus != 0 {
		m["vcpus"] = s.Vcpus
